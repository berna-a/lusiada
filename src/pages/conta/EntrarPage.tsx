...
type Etapa = "dados" | "confirmar-email";
...
function traduzirErro(bruto: string, modo: Modo): string {
  ...
  if (m.includes("invalid code") || m.includes("verification")) {
    return "O código não é válido ou já expirou. Confirme o email e tente outra vez.";
  }
...
export default function EntrarPage({ modoInicial }: { modoInicial: Modo }) {
  ...
  const submeter = async (e: FormEvent) => {
    ...
    setAEnviar(true);
    try {
      const resultado = await signIn("password", {
        email: mail,
        password: passe,
        flow: modo === "criar" ? "signUp" : "signIn",
      });
      if (modo === "criar" && !resultado.signingIn) {
  ...
  const confirmarEmail = async (e: FormEvent) => {
    ...
    if (codigoLimpo.length !== 6) {
      setErro("Escreva o código de seis dígitos que recebeu por email.");
      return;
    }
    setAEnviar(true);
    try {
      ...
      if (!resultado.signingIn) {
        setErro("Não foi possível confirmar o email. Peça um novo código.");
        return;
      }
      navigate(destino, { replace: true });
    } catch (err) {
      setErro(traduzirErro(err instanceof Error ? err.message : "", "criar"));
    } finally {
      setAEnviar(false);
    }
  };
    ...
    >
      ...
      <header className="text-center">
        ...
        <p className="mx-auto mt-5 max-w-[340px] font-body text-[15px] text-foreground/70 leading-relaxed">
          {etapa === "confirmar-email"
            ? `Enviámos um código de seis dígitos para ${email.trim().toLowerCase()}. Escreva-o abaixo para activar a conta.`
            : modo === "criar"
      ...
      {etapa === "confirmar-email" ? (
        <form onSubmit={confirmarEmail}>
          <label className="mt-8 block">
            <span className={rotulo}>Código de confirmação</span>
            <input
              autoComplete="one-time-code"
              className={`${campo} text-center font-mono text-[22px] tracking-[0.35em]`}
              inputMode="numeric"
              maxLength={6}
          ...
          <button
            className="mt-5 flex w-full items-center justify-center gap-2 font-body text-[14px] text-muted-foreground hover:text-primary"
            onClick={() => {
              setEtapa("dados");
              setCodigo("");
              setErro(null);
          ...
          >
            ...
            Corrigir o email ou pedir novo código
          </button>
        </form>
      ) : (
        <>
